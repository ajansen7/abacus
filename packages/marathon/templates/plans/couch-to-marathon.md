...
Pick this template when the user has no consistent running base in the last 90 days. Strong signals from plan-context: phrases like "starting from scratch," "couch to marathon," "haven't been running," or "first marathon." Strong signals from backfilled Strava: fewer than ~10 running activities in the 90 days before the plan's `startDate`, or running totals under ~10 km/week.
...
- Phases: weeks 0–25% are walk-heavy run/walk intervals. Weeks 25–60% are continuous easy running, building duration. Weeks 60–85% adds one tempo-lite session per week (no intervals at this template's intensity level). Final 15% is taper.

## Workout shape rules

- Easy runs cap at 70% of long-run duration.
- Long run grows by no more than 10 minutes per week and never exceeds 180 min (3 hr) for this template.
